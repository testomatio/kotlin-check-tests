...
Command-line interface for the Testomat.io Kotlin Reporter library.
...
```kotlin
    ...
    class CsvFileSourceParameterizedTests {

        @ParameterizedTest
        @CsvFileSource(
            resources = ["/com/library/model/junit/parameterizedtests/users.csv"],
            numLinesToSkip = 1
        )
        @TestId("d32625e6") // <== TestId added(or updated)
        fun testUserDataValidation(username: String, age: Int, email: String) {
...
```bash
    ...
    java -jar testomatio.jar clean-ids --directory ./src/test/kotlin
```
---
...
```bash
  export TESTOMATIO=... && \
  curl -L -O https://github.com/testomatio/kotlin-check-tests/releases/latest/download/testomatio.jar && \
